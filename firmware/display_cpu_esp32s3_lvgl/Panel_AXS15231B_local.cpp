...
namespace lgfx
{
    inline namespace v1
    {
        ...
        {
            // ESP_LOGD("AXS15231B","pannel init %d", use_reset);
            ...
            startWrite();
            for(int i=0;i<(sizeof(screen_init_cmds) / sizeof(screen_init_cmds[0]));i++)
            {
                cs_control(false);
                this->write_cmd(screen_init_cmds[i].cmd);
                for(int loop=0;loop<screen_init_cmds[i].data_bytes;loop++) {
                    _bus->writeCommand(((uint8_t *)screen_init_cmds[i].data)[loop], 8);
                }
                _bus->wait();
                cs_control(true);
                delay(screen_init_cmds[i].delay_ms + 1);
            }
            endWrite();
            delay(120);
        ...
        void Panel_AXS15231B::update_madctl(void)
        {
        ...
        void Panel_AXS15231B::setInvert(bool invert)
        {
            // ESP_LOGD("AXS15231B","setInvert %d", invert);
        ...
        void Panel_AXS15231B::setSleep(bool flg)
        {
            //ESP_LOGD("AXS15231B","setSleep %d", flg);
        ...
        void Panel_AXS15231B::setPowerSave(bool flg)
        {
            //ESP_LOGD("AXS15231B","setPowerSave");
        }


        void Panel_AXS15231B::waitDisplay(void)
        {
            //ESP_LOGD("AXS15231B","waitDisplay");
        }


        bool Panel_AXS15231B::displayBusy(void)
        {
            //ESP_LOGD("AXS15231B","displayBusy");
            return false;
        }

        color_depth_t Panel_AXS15231B::setColorDepth(color_depth_t depth)
        {
            //ESP_LOGD("AXS15231B","setColorDepth %d", depth);
        ...
        void Panel_AXS15231B::beginTransaction(void)
        {
            //ESP_LOGD("AXS15231B","beginTransaction");
        ...
        void Panel_AXS15231B::endTransaction(void)
        {
            //ESP_LOGD("AXS15231B","endTransaction");
            // if (!_in_transaction) return;
        ...
        void Panel_AXS15231B::write_bytes(const uint8_t* data, uint32_t len, bool use_dma)
        {
            start_qspi();
        ...
        void Panel_AXS15231B::setWindow(uint_fast16_t xs, uint_fast16_t ys, uint_fast16_t xe, uint_fast16_t ye)
        {
            //ESP_LOGD("AXS15231B","setWindow %d %d %d %d", xs, ys, xe, ye);
        ...
        void Panel_AXS15231B::writeBlock(uint32_t rawcolor, uint32_t len)
        {
            //ESP_LOGD("AXS15231B","writeBlock 0x%lx %ld", rawcolor, len);
        ...
        void Panel_AXS15231B::writePixels(pixelcopy_t* param, uint32_t len, bool use_dma)
        {
            //ESP_LOGD("AXS15231B","writePixels %ld %d", len, use_dma);
        ...
        void Panel_AXS15231B::drawPixelPreclipped(uint_fast16_t x, uint_fast16_t y, uint32_t rawcolor)
        {
            //ESP_LOGD("AXS15231B","drawPixelPreclipped %d %d 0x%lX", x, y, rawcolor);
        ...
        void Panel_AXS15231B::writeFillRectPreclipped(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, uint32_t rawcolor)
        {
            //ESP_LOGD("AXS15231B","writeFillRectPreclipped %d %d %d %d 0x%lX", x, y, w, h, rawcolor);
        ...
        void Panel_AXS15231B::writeImage(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, pixelcopy_t* param, bool use_dma)
        {
            //ESP_LOGD("AXS15231B","writeImage %d %d %d %d %d", x, y, w, h, use_dma);
            // use_dma = false;
        ...
        uint32_t Panel_AXS15231B::readCommand(uint_fast16_t cmd, uint_fast8_t index, uint_fast8_t len)
        {
            //ESP_LOGD("AXS15231B","readCommand");
            return 0;
        }

        uint32_t Panel_AXS15231B::readData(uint_fast8_t index, uint_fast8_t len)
        {
            //ESP_LOGD("AXS15231B","readData");
            return 0;
        }

        void Panel_AXS15231B::readRect(uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, void* dst, pixelcopy_t* param)
        {
            //ESP_LOGD("AXS15231B","readRect");
        }
        //----------------------------------------------------------------------------
    }
}
