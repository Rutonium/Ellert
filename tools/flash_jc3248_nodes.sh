...
echo "[Ellert] Input port: $ELLERT_INPUT_PORT"
...
arduino-cli upload -p "$ELLERT_INPUT_PORT" --fqbn "$FQBN" "$ROOT_DIR/firmware/input_cpu_esp32s3"
arduino-cli upload -p "$ELLERT_DISPLAY_PORT" --fqbn "$FQBN" "$ROOT_DIR/firmware/display_cpu_esp32s3"

echo "[Ellert] Flash complete."
